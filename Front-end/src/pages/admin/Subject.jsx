...
const SubjectsPage = () => {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      ...
      setIsChecking(true);
      try {
        const isCodeExist = subjects.some(
          (s) =>
            s.code === form.code.trim().toUpperCase() && s.id !== editTarget?.id
        );
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn xóa? Toàn bộ lớp học phần liên quan sẽ mất!"
      )
    )
      return;
    try {
    ...
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "20px" }}>
      ...
      >
        <div>
          ...
          <p style={{ color: "var(--text3)", margin: "5px 0 0" }}>
            Hệ thống quản lý đào tạo & tín chỉ VKU
          </p>
        </div>
      ...
      <div className="grid-3" style={{ marginBottom: 30, gap: 20 }}>
        {[
          ...
          {
            label: "Tổng tín chỉ",
            value: subjects.reduce((sum, s) => sum + (s.credits || 0), 0),
            color: "#10B981",
          },
          {
            label: "Lớp học phần",
          ...
          <div
            key={i}
            className="card"
            style={{ padding: 20, borderLeft: `5px solid ${stat.color}` }}
          >
      ...
      {loading ? (
        ...
        <div className="grid-2" style={{ gap: 20 }}>
          {filteredSubjects.map((s) => (
            <div
              key={s.id}
              className="card shadow-sm subject-card"
